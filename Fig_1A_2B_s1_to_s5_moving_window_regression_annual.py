...
DIR_SUMMARY = os.path.join(OUTPUT_ROOT, "summary")

HIST_LABEL_FONTSIZE = 12
HIST_TICK_FONTSIZE = 11
HIST_TITLE_FONTSIZE = 13
HIST_BOX_FONTSIZE = 11
    ...
    with rasterio.open(chlor_raster_path) as src_c, rasterio.open(emission_raster_path) as src_e:
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                slope, intercept, r_value, p_value, _ = linregress(b_vals, a_vals)
...
def _weighted_quantile(values, quantiles, weights):
    ...
    v, w = v[m], w[m]
    if v.size == 0:
        return np.array([np.nan] * len(q), dtype=float)
    srt = np.argsort(v)
    v, w = v[srt], w[srt]
    cw = np.cumsum(w)
    cw = cw / cw[-1]
...
def _area_weighted_mean_std(values, weights):
    v = np.asarray(values, dtype=float)
    w = np.asarray(weights, dtype=float)
    m = np.isfinite(v) & np.isfinite(w) & (w > 0)
    v, w = v[m], w[m]
...
def extract_nox_values_and_area_weights(df, emission_raster_path, xres, yres):
    if df.empty:
        return np.array([]), np.array([])
    coords = [(lon, lat) for lon, lat in zip(df["Longitude"].values, df["Latitude"].values)]
    with rasterio.open(emission_raster_path) as src:
        samples = list(src.sample(coords))
        nox_vals = np.array([s[0] if (s is not None and len(s) > 0) else np.nan for s in samples], dtype=float)
    area_w = np.array([pixel_area_m2(lat, xres, yres) / 1e12 for lat in df["Latitude"].values], dtype=float)
    m = np.isfinite(nox_vals) & np.isfinite(area_w) & (area_w > 0)
    return nox_vals[m], area_w[m]


def summarize_nox(nox_vals, area_w):
    nox_vals = np.asarray(nox_vals, dtype=float)
    area_w = np.asarray(area_w, dtype=float)
    m = np.isfinite(nox_vals) & np.isfinite(area_w) & (area_w > 0)
    nox_vals, area_w = nox_vals[m], area_w[m]
    if nox_vals.size == 0:
        return {"PixelCount": 0, "TotalArea_1e6km2": 0.0,
                "Mean_w": np.nan, "SD_w": np.nan, "Median_w": np.nan,
                "Q25_w": np.nan, "Q75_w": np.nan, "Min": np.nan, "Max": np.nan}
    mu, sd = _area_weighted_mean_std(nox_vals, area_w)
    q25, med, q75 = _weighted_quantile(nox_vals, [0.25, 0.50, 0.75], area_w)
    return {"PixelCount": int(nox_vals.size), "TotalArea_1e6km2": float(np.sum(area_w)),
            "Mean_w": float(mu), "SD_w": float(sd), "Median_w": float(med),
            "Q25_w": float(q25), "Q75_w": float(q75),
            "Min": float(np.nanmin(nox_vals)), "Max": float(np.nanmax(nox_vals))}


# Main
def main():
    for d in [DIR_STATIC_R2, DIR_STATIC_R, DIR_STATIC_SLOPE, DIR_SHP, DIR_SUMMARY]:
    ...
    chlor_files = sorted(glob.glob("data/chlor/chlor_*.tif"))
    ...
    for chlor_path in chlor_files:
        ...
        shp_pos_out = os.path.join(DIR_SHP, f"moving_window_regression_{year}_pos.shp")
        ...
        shp_neg_out = os.path.join(DIR_SHP, f"moving_window_regression_{year}_neg.shp")
        ...
        shp_r_all_out = os.path.join(DIR_SHP, f"moving_window_regression_{year}_r_all.shp")
        ...
        shp_r_pos_out = os.path.join(DIR_SHP, f"moving_window_regression_{year}_r_pos.shp")
        ...
        shp_r_neg_out = os.path.join(DIR_SHP, f"moving_window_regression_{year}_r_neg.shp")
        gdf_r_neg.to_file(shp_r_neg_out)
        print(f"  Saved r < 0 shapefile: {shp_r_neg_out}")

        # Static maps
        png_r2_out = os.path.join(DIR_STATIC_R2, f"moving_window_regression_map_{year}.png")
        visualize_static_map_with_regression(
            df, png_r2_out,
            f"Chlor-NOx Moving Window Regression {year} (R2, R2 >= 0.6)",
            xres, yres)

        png_r_out = os.path.join(DIR_STATIC_R, f"moving_window_regression_map_{year}_r.png")
        visualize_static_map_with_r(
            df, png_r_out,
            f"Chlor-NOx Moving Window Regression {year} (r, R2 >= 0.6)",
            xres, yres)

        png_slope_out = os.path.join(DIR_STATIC_SLOPE, f"moving_window_regression_map_{year}_slope.png")
        visualize_static_map_with_slope(
            df, png_slope_out,
            f"Chlor-NOx Moving Window Regression {year} (slope, R2 >= 0.6)",
            xres, yres)
        ...
        year_nox, year_area_w = extract_nox_values_and_area_weights(df_pos_hist, emission_path, xres, yres)
        s = summarize_nox(year_nox, year_area_w)
        summary_rows.append({"Year": year, **s})

        if year_nox.size > 0:
            pooled_nox.append(year_nox)
            pooled_area_w.append(year_area_w)

    if total_areas:
        avg_area = sum(total_areas) / len(total_areas)
        print(f"Average total area across years = {avg_area:.2f} km2")

    # Pooled summary and Excel
    pooled_label = "2013-2017"
    excel_out = os.path.join(DIR_SUMMARY, "nox_summary_2013_2017.xlsx")

    if pooled_nox:
        pooled_nox_all = np.concatenate(pooled_nox)
        pooled_area_all = np.concatenate(pooled_area_w)
        pooled_summary = summarize_nox(pooled_nox_all, pooled_area_all)
        summary_rows.append({"Year": pooled_label, **pooled_summary})
    else:
        summary_rows.append({"Year": pooled_label, **summarize_nox(np.array([]), np.array([]))})
        print("[Pooled] No pooled NOx samples.")

    summary_df = pd.DataFrame(summary_rows)
    years = ["2013", "2014", "2015", "2016", "2017", pooled_label]
    summary_df["Year"] = pd.Categorical(summary_df["Year"], categories=years, ordered=True)
    ...
    with pd.ExcelWriter(excel_out, engine="openpyxl") as writer:
        ...
        if pooled_nox:
            raw_df = pd.DataFrame({
                "NOx_emission": pooled_nox_all.astype(float),
                "AreaWeight_1e6km2": pooled_area_all.astype(float),
            })
            raw_df.to_excel(writer, index=False, sheet_name="PooledRaw")

    print(f"[Saved] Excel summary: {excel_out}")

    pooled_row = summary_df[summary_df["Year"] == pooled_label]
    if not pooled_row.empty:
        r = pooled_row.iloc[0]
        print(f"\n[REPORT - POOLED 2013-2017 | positive-slope & R2>=0.6]")
        print(
            f"NOx emission: median={r['Median_w']:.4g} (IQR {r['Q25_w']:.4g}-{r['Q75_w']:.4g}) "
            f"g m-2 yr-1; "
            f"area-weighted mean={r['Mean_w']:.4g} +/- {r['SD_w']:.4g}; "
            f"total area={r['TotalArea_1e6km2']:.2f} x10^6 km2; pixels={int(r['PixelCount'])}."
        )
